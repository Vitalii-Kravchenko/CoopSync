...
export const ru: Translation = {
  ...
  main: {
    ...
    pushSkippedNoChangeExit: 'Проверено при выходе — нечего выгружать, всё уже было синхронизировано',
    statusesError: 'Не удалось проверить статус синхронизации',
    retry: 'Попробовать снова',
    addGameCard: 'Добавить игру'
  },
  gameCard: {
  ...
  addGame: {
    title: 'Добавить игру',
    description:
      'Этой игры нет во встроенном каталоге CoopSync, поэтому вся папка сохранений копируется как есть — мы не знаем, какие файлы на самом деле являются сохранениями. Укажи .exe игры ниже — и автосинхронизация при запуске/выходе будет работать так же, как для игр из каталога; если пропустишь этот шаг, синхронизация будет только ручной, через кнопки Upload/Download. Если в той же папке лежат локальные настройки или файлы аккаунта — они тоже синхронизируются.',
    nameLabel: 'Название игры',
    namePlaceholder: 'напр. Моя игра',
    pathLabel: 'Папка сохранений',
    submit: 'Добавить игру',
    installPathLabel: 'Папка установки (необязательно)',
    installPathHint: 'Чтобы CoopSync нашёл .exe игры и включил автосинхронизацию при запуске/выходе.',
  ...
  history: {
    ...
    savePathEdit: 'Редактировать',
    savePathBrowse: 'Обзор…',
    savePathPlaceholder: 'Полный путь к папке сохранений',
    savePathSave: 'Сохранить',
    ...
    coverSyncRetrySuccess: 'Обложка синхронизирована.',
    excludeFilesTitle: 'Исключить файлы из синхронизации',
    excludeFilesHint:
      'Отметь здесь файлы, которые не должны синхронизироваться — локальные настройки, данные аккаунта, всё, что на самом деле не является сохранением. В списке только файлы прямо в папке сохранений (без подпапок). Этот выбор действует только на этом ПК — он не передаётся другу, потому что точные названия файлов могут отличаться между установками. Друг должен исключить файлы отдельно, ориентируясь на то, что видит в своей папке.',
    excludeFilesEmpty: 'В папке сохранений пока нет файлов.',
    excludeFilesRefresh: 'Обновить список файлов',
    extraFoldersTitle: 'Дополнительные папки сохранений',
    extraFoldersHint:
      'Синхронизируй для этой игры ещё одну папку отдельно — например, сохранения персонажей отдельно от сохранений миров. Отметь папку «Только я» — она всё равно будет бэкапиться в облако, но в твоё личное пространство, которое друг никогда не увидит.',
    extraFoldersEmpty: 'Дополнительных папок пока нет',
    extraFoldersAdd: 'Добавить папку',
    extraFolderLabelPlaceholder: 'Название (напр. Персонажи)',
    extraFolderPathPlaceholder: 'Путь к папке',
    extraFolderBrowse: 'Обзор',
    extraFolderShared: 'Общая с другом',
    extraFolderPersonal: 'Только я',
    ...
    extraFolderNoPath: 'Папка не указана',
    extraFolderRemove: 'Удалить папку',
    extraFolderRemoveConfirmTitle: 'Удалить эту папку?',
    extraFolderRemoveConfirmDesc: (label) =>
      `Прекратить синхронизацию «${label}»? Уже сохранённое в облаке не удаляется.`,
    extraFolderRemoveError: 'Не удалось удалить папку',
  ...
  presence: {
    title: 'Присутствие',
    description:
      'Мгновенно видеть, когда друг по коопу онлайн, и получать уведомление сразу, как только он выгрузит сейв — вместо ожидания периодической проверки. Опционально, по умолчанию выключено.',
    enable: 'Включить присутствие',
    disable: 'Выключить',
    statusOff: 'Выключено',
  ...
  errors: {
    ...
    GAME_NAME_TAKEN: () => 'Игра с таким названием уже есть — выбери другое имя',
    ...
    NOT_FOLDER_OWNER: () => 'Изменить общий доступ может только тот, кто добавил эту папку',
    FOLDER_NAME_TAKEN: () => 'Папка с таким названием для этой игры уже есть — выбери другое имя',
    FOLDER_PATH_OVERLAPS: (p) =>
      `Эта папка (или папка внутри/над ней) уже участвует в синхронизации как часть «${p.with}» — выбери ещё не занятую папку`
  }
}
